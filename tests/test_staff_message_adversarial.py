...
def test_staff_reply_rejects_message_from_another_claim_without_mutation(
    client: TestClient,
    auth_headers: dict[str, str],
    staff_auth_headers: dict[str, str],
    repository: FixtureRepository,
    ...
    claim_id, active_session_id, handoff_id, revision = _queued_handoff(
        client, auth_headers, 'cross-claim-reply'
    )
    accepted_revision = _accept(
        client,
        staff_auth_headers,
    ...
    other_message = client.post(
        ...
        headers={
            **auth_headers,
            ...
            'If-Match': str(other_body['claim']['revision']),
        },
        json={
    ...
    other_message_id = cast(str, other_message.json()['claimant_message']['message_id'])
    ...
    response = client.post(
        ...
        headers={
            **staff_auth_headers,
            'Idempotency-Key': 'cross-claim-reply-send',
            'If-Match': str(accepted_revision),
        },
        json={
    ...
    assert after.revision == accepted_revision
    assert (
        repository.list_messages(claim_id, active_session_id, claim.customer_id)
        == before_messages
    )
    assert repository.list_handoffs(claim_id, claim.customer_id) == before_handoffs
    assert repository.find_idempotency('stf_demo', route, 'cross-claim-reply-send') is None
