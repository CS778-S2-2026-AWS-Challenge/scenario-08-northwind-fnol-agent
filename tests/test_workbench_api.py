...
def test_workbench_list_returns_claim_summaries(
    client: TestClient,
    auth_headers: dict[str, str],
    staff_auth_headers: dict[str, str],
) -> None:
    create_response = client.post(
        '/api/v1/claims',
        headers={**auth_headers, 'Idempotency-Key': 'workbench-list-claim'},
        json={
            'channel': 'web_agent',
            'locale': 'en-NZ',
            'incident_type': 'motor',
    ...
    assert items[0]['workflow_state'] == 'created'
    assert 'sessions' not in items[0]
