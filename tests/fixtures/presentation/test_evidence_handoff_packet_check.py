...
def test_the_packet_classifies_visibility_by_source(
    ...
    """The packet applies a record-level visibility rule.

    `default_handoff_visibility` marks a claimant-supplied record `shared` and
    everything else `internal_only`. This is the rule the claimant evidence
    list does not yet apply — see the finding recorded in
    `docs/day5-evidence-handoff-packet.md`.
    """
    with client as active:
    ...
    # The same rule on a record the claimant did not supply.
    staff_record = record.model_copy(
        update={'evidence_id': 'evd_staff_probe', 'source': EvidenceSource.STAFF}
    )
