...
def test_evidence_id_cannot_be_reowned_by_another_claim() -> None:
    ...
    assert (
        repository.get_evidence(first.claim_id, original.evidence_id, first.customer_id) == original
    )
    assert (
        repository.get_evidence(second.claim_id, original.evidence_id, second.customer_id) is None
    )
