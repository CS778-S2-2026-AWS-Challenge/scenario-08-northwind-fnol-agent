...
def test_claimant_handoff_staff_reply_and_continuation_share_one_authoritative_claim(
    ...
    staff_message = next(item for item in visible_messages if item['message_id'] == staff_message_id)
    ...
    assert any(message.client_message_id == 'd4-267-continuation' for message in stored_messages)
