...
| --- | --- | --- |
| Live model-backed intake | The provider-neutral gateway and explicit `model_gateway` profile are implemented. On 14 September 2026, the nowcoding `gpt-5.6-terra` endpoint returned valid strict structured output, a forced tool call with valid arguments, and a successful claimant Runtime turn through the published model and complete Agent-policy Release Set. The profile is published as `nowcoding-gpt56terra`; credentials remain outside source and persisted configuration. This evidence verifies that profile and path only, not every provider, all ten scenarios, or production availability. | @Ysoseri1224 |
