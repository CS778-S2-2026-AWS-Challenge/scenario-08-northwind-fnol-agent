...
| --- | --- | --- |
| Live model-backed intake | The provider-neutral gateway and explicit `model_gateway` profile are implemented. On 15 September 2026, the nowcoding `gpt-5.5` endpoint returned valid strict structured output and a forced tool call with valid arguments. The profile is published locally as `nowcoding-gpt55`; credentials remain outside source and persisted configuration. A pre-fix claimant turn exposed Prompt/Runtime drift rather than provider failure, so provider probes alone are not recorded as a successful claimant journey. Prompt v6 and the governed publication path close that contract defect; a new complete live turn remains separate deployment evidence. | @Ysoseri1224 |
...
| Admin API and Control Plane configuration | The authenticated `/internal/v1/admin` surface now exposes versioned configuration, Release Sets/runtime snapshots, accounts, integrations, operations, evaluations, knowledge, Agent-rule components, access-policy records, and bounded audit search. Published access policies can restrict existing server-side scopes; the Admin Console provides read-only projections. Dynamic role assignment and full mutation screens remain open gaps documented in `docs/status/control-plane-progress.md`. | @Ysoseri1224 |
| Persisted identity access audit | Customer and staff account updates create immutable administration audit events, and account projections are database-backed. Authentication-denial events and a complete policy-driven authorization audit stream remain outside the current slice. | @jxu316-arch |
