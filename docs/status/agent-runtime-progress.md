...
| Stable Request Profiles, narrow schemas, and provider capabilities | implemented | v7 registries, OpenAI strict-schema and Bedrock forced-output materialization tests | Bedrock lookup continuation remains unavailable and fails compatibility before transport. |
...
| Verified rolling summary and lazy history | implemented | Fixture/Mongo immutable summary persistence, background compaction, older-message resolver, and focused parity tests | Compaction is post-response; conflicting summaries are omitted in favour of Claim State. |
| Isolated PDF, multi-Evidence, and cross-Claim review | implemented | isolated executor and v7 focused tests | Results are sourced read-only summaries and cannot mutate Claim State, consent, or external services. |
