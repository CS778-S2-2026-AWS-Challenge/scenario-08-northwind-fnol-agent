...
| Claimant Evidence history Agent boundary | implemented (proposal and execution-port boundary) | `evidence.history`, proposal and confirmed action contracts, `backend/services/agent_tools.py`, `backend/services/agent_evidence_actions.py`, and focused contract tests | History remains claimant-scoped and read-only. Confirmed Runtime actions validate scope, provenance, confirmation, revision, and idempotency and preserve five typed outcomes. No production handler is bound; persisted attachment/removal remains unavailable until the Evidence API publishes governed operations. |
