...
| Session-bound dual model catalog | implemented | `backend/services/model_profiles.py`, `tests/test_namespaced_runtime.py`, claimant capabilities contract | The fixture/bootstrap catalog is not a remote provider health check; live evidence for both profiles remains deployment work. |
| Namespaced `claim.read` tool loop | implemented (minimal) | `backend/services/model_agent.py`, `backend/services/agent_tools.py`, `tests/test_namespaced_runtime.py` | Only `claim.read` and `conversation.answer`/`runtime.continue` are wired; other tools and target actions remain unavailable. |
| Read-only Runtime trace persistence | implemented (minimal) | `RuntimeTraceRecord`, fixture/Mongo `save_runtime_turn`, idempotency replay test | A complete TurnPlan/ExecutionPlan/TurnResult record family and admin trace projection remain open. |
...
| Namespaced Agent Action Registry | implemented | `backend/domain/agent_action_registry.py` and `tests/test_agent_action_registry.py`, issue #365 | The registry defines the target vocabulary and validation boundaries; only the minimal Runtime continuation is currently executable. |
