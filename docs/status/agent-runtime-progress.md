...
| Claimant Evidence history Agent boundary | implemented (bounded proposal slice) | `evidence.history`, `claim.propose_evidence_reuse`, `claim.propose_evidence_remove`, `backend/services/agent_tools.py`, and Agent/tool-policy contract tests | History lookup is claimant-scoped and read-only. Reuse/removal remain proposals; persisted attachment/removal handlers are unavailable until the Evidence API publishes governed operations. |
