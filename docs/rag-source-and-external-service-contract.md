...
authority is not yet modelled. The #262 claimant flow records that bounded consent,
derives the authorised request from shared state, and shows provider, shared-data,
progress, success, and failure states without exposing internal references. The exact
external Challenge Key Feature 2 wording is absent from the repository, so the scenario
records that reference as `unverified_external_brief` rather than claiming complete
