...
- An external task result records what the third party returned, which is a different fact from
  the acknowledgement that a request reached it. It is stored only against a task whose
  `delivery` is `submitted`: nothing reached a provider otherwise, so no provider answer can
  exist, and a routing or request acknowledgement is not an answer. The task's
  `provider_reference` remains a provider reference and is never the result.
