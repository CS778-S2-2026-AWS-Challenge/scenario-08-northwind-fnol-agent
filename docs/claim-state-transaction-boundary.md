...
| --- | --- | --- |
| Resume/start claimant session | claim + new/updated session + idempotency | claim `N -> N+1`; session linked to the same claim/customer; one active claimant session |
| Message-only claimant continuation | claim + session + message + idempotency | claim `N -> N+1`; session context updated to the resulting claim revision |
...
3. **Validate next revision** is exactly `expected_revision + 1`.
