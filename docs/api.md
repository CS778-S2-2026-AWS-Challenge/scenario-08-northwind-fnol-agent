...
server returns `MODEL_PROFILE_UNAVAILABLE`. A resumed Session keeps the last selected profile by
default, and the profile may be changed explicitly on a later message when the replacement is
...
The optional `model_profile_id` in a start/resume request is validated against the published
catalog. Resume keeps the current Session profile unless a new available profile is explicitly
selected; the server never silently replaces an unavailable or withdrawn profile with another
model.
...
`client_message_id` is generated by the client and deduplicates retries within the claim. The
optional `model_profile_id` selects any currently published claimant profile for this turn; it
...
WorkItem records are retained for authorised audit/readback. The deprecated eight-action model
contract is rejected with `LEGACY_AGENT_ACTION_DEPRECATED`; it is never mapped to a namespaced
action or controlled fallback.
...
| `GET` | `/workbench/claims/{claim_id}/runtime-work-items` | Read claimant Runtime WorkItems reconciled against current Claim facts |
...
Agent `session_id`, assistant `message_id`, `draft_id`, and `staff_agent_execution_id` alongside the
registered action and resulting Claim revision. The same atomic mutation writes an immutable
`StaffAgentExecutionRecord` containing the confirmed source, registered action, expected and
resulting Claim revisions, and authoritative handler result. Persistence rejects the record unless
the referenced session belongs to the acting staff member and the referenced assistant message
contains exactly that Claim-scoped draft with the same action and target. Reusing an idempotency key
with another draft, invoking the same draft through another key, or using a non-Agent route returns
`409 IDEMPOTENCY_CONFLICT`.

The successful response includes this record as `runtime_execution`; it is returned only to the
authenticated staff execution route and is never part of a claimant projection. The response is a
readback of persisted evidence rather than an independent success assertion. If the handler result
cannot be read back through that record, the endpoint fails closed instead of constructing a
success response from the model draft.
...
| Claimant Runtime WorkItems | `GET /workbench/claims/{claim_id}/runtime-work-items` |
...
## Claimant policy summaries, assets, and Claim snapshots
...
| --- | --- |
| `POST /api/v1/account/policies` | Create a bounded Policy Summary. Requires `Idempotency-Key`; returns `201`. The input is account display data, not a coverage result. |
| `GET /api/v1/account/policies` | Cursor-page active owned summaries; `include_inactive=true` includes soft-deactivated records. |
| `GET /api/v1/account/policies/{policy_id}` | Read one owned claimant-safe Policy Summary. |
| `PATCH /api/v1/account/policies/{policy_id}` | Update `policy_number` or `display_name` with numeric `If-Match`; product family and verification status are not claimant-editable through this route. |
| `DELETE /api/v1/account/policies/{policy_id}` | Soft-deactivate with numeric `If-Match`; returns `204`. Existing Claim snapshots remain unchanged. |
...
Policy Summary identifiers use `pol_`; Asset identifiers use `ase_`; snapshots use `cas_`. A
Policy Summary contains `policy_number`, `display_name`, `product_family`,
`verification_status`, `revision`, `active`, and timestamps. It contains no coverage outcome,
policy wording, provider credential, or payment data. The account-safe projection omits
`customer_id` and physical storage/provider metadata.

Asset records contain `asset_type`, `display_name`, matching typed `details`, optional
`policy_id`, `revision`, `active`, and timestamps. An association is accepted only when `policy_id`
resolves to an active Policy Summary owned by the same claimant and its product family matches the
Asset type. Arbitrary policy text, cross-owner references, inactive summaries, and family
mismatches are rejected. Clearing `policy_id` uses explicit `null` on Asset PATCH; other Asset
fields remain non-null.

Selection returns `claim_id`, resulting `revision`, the exact `proposed_fields`, and the
immutable snapshot. A linked summary additionally proposes `policy.policy_number` from the Policy
source reference and copies the bounded Policy association into the immutable snapshot. It never
silently confirms a field or coverage. A later policy or Asset update/deactivation does not alter a
snapshot. The selection idempotency identity includes the Claim ID, request payload,
...
facts retain only bounded identities and source references, not copied Asset or policy values.
...
`/api/v1/claims/{claim_id}/participants`,
...
- current Agent Decisions, TurnPlans, AgentProposals, ExecutionPlans, ActionEnvelopes,
  ToolRequests and results, TurnResults, and Staff Agent execution records;
