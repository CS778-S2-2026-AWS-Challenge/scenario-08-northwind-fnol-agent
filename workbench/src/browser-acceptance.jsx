...
const missingInformation = Array.from({ length: 8 }, (_, index) => ({
  ...
  attention: index < 2 ? 'blocking' : 'needed_next',
...
const detail = {
  ...
  priority_projection: { level: 'elevated' },
  work_summary: {
    ...
    risk_signals: Array.from({ length: 5 }, (_, index) => ({ signal_id: `sig_${index + 1}`, label: `Review signal ${index + 1}`, attention_level: 'review', summary: 'Source-linked review context.' })),
  },
