...
def _ensure_local_bucket(config: S3CompatibleObjectStorageConfig) -> None:
...
def main() -> None:
    ...
    if evidence_items[0]['file_status'] != 'processing':
        raise RuntimeError('The persisted evidence did not enter the expected processing state.')
