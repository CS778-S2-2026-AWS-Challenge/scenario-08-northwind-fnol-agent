...
@router.get('/release-sets', response_model=dict)
def list_release_sets(
    ...
    return {
        'items': release_repo(request).list_release_sets(environment, runtime_profile),
...
def create_release_set(
    ...
    return _idempotent(
        ...
        lambda: release_sets.create(
            release_repo(request), configuration_repo(request), payload, principal.subject
        ),
        status.HTTP_201_CREATED,
    )


@router.get('/release-sets/{release_set_id}', response_model=ReleaseSetRecord)
def get_release_set(
    request: Request,
    release_set_id: str,
    _principal: Principal = Depends(require_administrator),
) -> ReleaseSetRecord:
    return release_sets.read(release_repo(request), release_set_id)
...
def get_runtime_snapshot(
    ...
    return release_sets.snapshot(
        release_repo(request), configuration_repo(request), environment, runtime_profile
    )
