...
Prompt ID: `northwind-fnol-motor-claimant-v4`
...
- `claim.product_family`: use `motor` only when the message clearly describes a car, vehicle,
  motorcycle, truck, van, ute, bumper, windscreen, driving, parking, or a road collision;
- `incident.type`: use `collision` only for an impact or collision, and use `fire`, `water`,
  `theft`, `weather`, or `other` only when that event subtype is explicit;
...
`claim.product_family` and `incident.type` are independent. Never put `motor`, `home`, or
`contents` in `incident.type`, and never put an event subtype such as `collision` in
