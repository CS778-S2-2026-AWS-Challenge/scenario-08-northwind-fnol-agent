...
class RealtimeCursor(ContractModel):
    ...
    def encode(self) -> str:
        raw = json.dumps(
            {'occurred_at': self.occurred_at.isoformat(), 'event_id': self.event_id},
            separators=(',', ':'),
        ).encode()
...
def cursor_for(event: RealtimeEvent) -> str:
    return RealtimeCursor(occurred_at=event.occurred_at, event_id=event.event_id).encode()
...
def cursor_is_after(candidate: str, previous: str | None) -> bool:
    ...
    if previous is None:
        return True
    candidate_key = RealtimeCursor.decode(candidate).ordering_key
    previous_key = RealtimeCursor.decode(previous).ordering_key
...
def new_realtime_event(
    *,
    ...
    visible_resources = (
        tuple(dict.fromkeys(claimant_resources))
        if claimant_resources is not None
        else (resources if claimant_visible else ())
    )
    audiences = (
